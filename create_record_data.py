    ...
    for entry in input_data:
        ...
        for qa in entry["qas"]:
            ...
            example = RecordExample(
                ...
                query_start_list=query_start_list,
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for tk in query_tokens:
            if type(tk) == type('a'.encode('utf-8')):
                tmp.append(tk.encode('utf-8'))
        ...
        if is_training:
            ...
            for idx in range(len(example.query_end_list)):
                tok_qstart = orig_to_tok_index[example.query_start_list[idx]]
                tok_qend = orig_to_tok_index[example.query_end_list[idx]]
                (tok_qstart, tok_qend) = _improve_answer_span(
                    all_doc_tokens, tok_qstart, tok_qend, tokenizer,
                    example.orig_answer_text)
                tok_estart.append(tok_qstart)
                tok_eend.append(tok_qend)
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if is_training:
                ...
                for idx in range(len(tok_estart)):
                    doc_start = doc_span.start
                    doc_end = doc_span.start + doc_span.length - 1
                    out_of_span = False
                    if not (tok_estart[idx] >= doc_start and
                            tok_eend[idx] <= doc_end):
                        out_of_span = True
                    if out_of_span:
            ...
            feature = InputFeatures(
                ...
                query_start_list=example.query_start_list,
                query_end_list=example.query_end_list,
                input_ids=input_ids,
...
def main(args):
    ...
    tokenizer = tokenization.FullTokenizer(vocab_file=None, spm_model_file=vocab_model, do_lower_case=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_file", default="data/dataset/record/train.json", type=str)
    parser.add_argument("--vocab_model", default="ALBERT/pretrained_model/albert-base-v2-spiece.model", type=str)
    parser.add_argument("--save_path", default="data/dataset/record/", type=str)
